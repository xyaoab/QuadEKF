...
void imu_callback(const sensor_msgs::Imu::ConstPtr &msg)
{
    ...
    visualizeText(Vector3d(1,1,2.5), "POS"+ std::to_string(x(0)) + " , " + std::to_string(x(1)) +" , "+std::to_string(x(2)), pub_text);
...
void odom_callback(const nav_msgs::Odometry::ConstPtr &msg) // pnp transfrom to vicon frame instead of mat
{
    ...
    //                                       0, 0, -1;
        ...
        //cout<<"PNP"<<phi<<" "<<theta<<" "<<psi<<endl;
        visualizeText(Vector3d(0,0,-1), "PNP"+ std::to_string(phi) + " , " + std::to_string(theta) +" , "+std::to_string(psi), pub_text);
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber s2 = n.subscribe("tag_odom", 1000, odom_callback,ros::TransportHints().tcpNoDelay());
    ros::Subscriber s3 = n.subscribe("optical_flow_odom", 1000, optical_flow_odom_callback,ros::TransportHints().tcpNoDelay());
    ...
    pub_text = n.advertise<visualization_msgs::Marker>("ekf_text", 1, true);
    Rcam << 0,-1,0,
            -1,0,0,
            0,0,-1;
    ...
    Q.topLeftCorner(6, 6) = 0.5 * Q.topLeftCorner(6, 6);
    Q.bottomRightCorner(6, 6) = 0.5 * Q.bottomRightCorner(6, 6);
    Q.bottomRightCorner(1, 1) = 1.3 * Q.bottomRightCorner(1, 1);
